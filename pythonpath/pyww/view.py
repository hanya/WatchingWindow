...
from com.sun.star.awt import XWindowListener, XKeyHandler, \
    XFocusListener, XActionListener, XMouseListener, ActionEvent
...
class WatchingWindowView(unohelper.Base, XWindowListener, XActionListener, 
    ...
    def __init__(self, ctx, model, frame, parent, res, show_input_line=False):
        self.ctx = ctx
        self.res = res
        self.model = model
        self.grid = None
        ...
        try:
            self.input_line_shown = show_input_line
            self._create_view(ctx, parent, res, show_input_line)
            self.grid.addSelectionListener(self)
        except Exception as e:
            print("Failed to create Watching Window view: %s" % e)
    ...
    def _create_view(self, ctx, parent, res, show_input_line=False):
        ...
        titles = (res["Sheet"], res["Cell"], res["Value"], res["Formula"])
    ...
    def select_entry(self, index):
        ...
        if index >= 0 and index < self.model.data_model.getRowCount():
    ...
    def is_selected_entry_moveable(self, up):
        ...
        if up and i > 0:
            return True
        elif not up and i < (self.model.data_model.getRowCount() - 1):
            return True
        return False
    ...
    def update_buttons_state(self):
        ...
        if self.model.data_model.getRowCount() == 0:
    ...
    def windowResized(self, ev):
        ...
        self.cont.setPosSize(0, 0, ps.Width, ps.Height, PS_SIZE)
        
        gc("spinner").setPosSize(
            ps.Width - self.BUTTON_WIDTH * 2 - self.RIGHT_MARGIN, 0, 0, 0, PS_X)
        gc("btn_option").setPosSize(
            ps.Width - self.BUTTON_WIDTH - self.RIGHT_MARGIN, 0, 0, 0, PS_X)
        
        if self.input_line_shown:
            gc("grid").setPosSize(
                0, 0, ps.Width, 
                ps.Height - self.BUTTON_HEIGHT - self.TOP_MARGIN * 3 - self.INPUT_LINE_HEIGHT, 
                PS_SIZE)
        else:
            gc("grid").setPosSize(
                0, 0, ps.Width, 
                ps.Height - self.BUTTON_HEIGHT - self.TOP_MARGIN * 2, PS_SIZE)
        gc("edit_input").setPosSize(
            0, 0, ps.Width - self.LEFT_MARGIN - self.RIGHT_MARGIN, 0, PS_WIDTH)
    ...
    def actionPerformed(self, ev):
        try:
            getattr(self, "cmd_" + ev.ActionCommand)()
        except Exception as e:
            print(e)
    ...
    def cmd_up(self):
        self.model.move_entry(True)
    
    def cmd_down(self):
        self.model.move_entry(False)
    ...
    def cmd_settings(self):
        ...
        settings = Settings(self.model.ctx, self.res)
        if settings.configure():
            self.model.WarnCells = settings.get("WarnNumberOfCells")
    ...
    def selectionChanged(self, ev):
        ...
        self.model.set_input_line()
    # XFocusListener
    def focusGained(self, ev):
        self.model.frame.getController().addKeyHandler(self)
    def focusLost(self, ev):
        self.model.frame.getController().removeKeyHandler(self)
    
    # XKeyHandler
    def keyPressed(self, ev):
        if ev.KeyCode == K_RETURN:
            self.model.update_row(self.get_input_text())
            return True
        return False
    ...
    def context_menu(self, x, y):
        ...
        if popup is None:
            popup = create_popup(self.model.ctx, 
                (MenuEntry(self.res["Go to Cell"], 4, 0, "goto"), 
                MenuEntry(self.res["Go to"], 6, 1, "gotocell"), 
                MenuEntry(self.res["Remove"], 8, 2, "delete"), 
                MenuEntry("", -1, 3), 
                MenuEntry(self.res["Up"], 10, 4, "up"), 
                MenuEntry(self.res["Down"], 11, 5, "down")
                ), True)
        ...
        if popup:
            ...
            state = False
            if index >= 0:
                cell = self.model.data_model.get(index).get_cell()
                refs = self.model.get_cell_references(cell)
                #print(refs)
                if refs:
                    items = []
                    for i, ref in enumerate(refs):
                        items.append(MenuEntry(ref, i + 1000, i, ""))
                    goto_popup = create_popup(self.model.ctx, items, False)
                    popup.setPopupMenu(6, goto_popup)
                    state = True
            ...
            n = popup.execute(
                    self.cont.getPeer(), 
                    Rectangle(x + ps.X, y + ps.Y, 0, 0), PMD_EXECUTE_DEFAULT)
            if n > 0 and n < 1000:
                cmd = popup.getCommand(n)
                self.actionPerformed(ActionEvent(None, cmd))
            elif n >= 1000:
                addr = refs[n - 1000]
                self.model.goto_cell(addr)
    
    def option_popup(self, x, y):
        """ Show popup menu for option button. """
        popup = create_popup(self.model.ctx, 
            (
                MenuEntry(self.res["Clear"], 32, 0, "clear"), 
                MenuEntry("", -1, 1, ""), 
                MenuEntry(self.res["Input line"], 1024, 2, "switch_inputline", style=MIS_CHECKABLE), 
                MenuEntry(self.res["Store watches"], 2048, 3, "switch_store", style=MIS_CHECKABLE), 
                MenuEntry("", -1, 4, ""), 
                MenuEntry(self.res["Settings..."], 512, 5, "settings"), 
                MenuEntry(self.res["About"], 4096, 6, "about"), 
            ), True)
    ...
    def messagebox(self, message, title, message_type, buttons):
        ...
        parent = self.model.frame.getContainerWindow()
    ...
    def switch_input_line(self, new_state=None):
        ...
        if new_state:
            self.grid.setPosSize(
                0, self.TOP_MARGIN * 3 + self.BUTTON_HEIGHT + self.INPUT_LINE_HEIGHT, 
                0, 
                ps.Height - (self.TOP_MARGIN * 3 + self.BUTTON_HEIGHT + self.INPUT_LINE_HEIGHT), 
                PS_Y + PS_HEIGHT)
            self.cont.getControl("edit_input").addFocusListener(self)
        else:
            self.grid.setPosSize(0, self.TOP_MARGIN * 2 + self.BUTTON_HEIGHT , 
            0, ps.Height - (self.TOP_MARGIN * 2 + self.BUTTON_HEIGHT), PS_Y + PS_HEIGHT)
    ...
    def _get_spinner(self):
        return self.cont.getControl("spinner")
